# STIGPilot GitHub Issue Drafts

## Issue 1

Title: `added: V-CHROME-004 - Chrome extension installation must be restricted`

Labels: `stigpilot`, `high_priority_review`, `GPO`, `Registry`, `Browser Security`

### Context

- Severity: high
- Suggested owner: Endpoint/Windows Admin
- Impact: High-priority review (`high_priority_review`)
- Reason: A new high-severity control was added, so it should be triaged before lower-risk backlog work.
- Changed fields: added

### Evidence Needed

- [ ] Screenshot or export of the relevant setting
- [ ] GPO, registry, or Local Security Policy export
- [ ] Date/time of validation
- [ ] System or asset name
- [ ] Reviewer notes

### Acceptance Criteria

- [ ] Change reviewed by suggested owner
- [ ] Ticket priority matches STIGPilot impact category
- [ ] Evidence request updated or confirmed unchanged
- [ ] Notes added for any downstream checklist or backlog impact

### Notes

- 

## Issue 2

Title: `removed: V-CHROME-003 - Deprecated Chrome cleanup policy must be reviewed`

Labels: `stigpilot`, `review_recommended`, `Browser Security`

### Context

- Severity: low
- Suggested owner: Security/GRC Analyst
- Impact: Review recommended (`review_recommended`)
- Reason: The control was removed, so downstream tickets, evidence requests, or mappings may need cleanup.
- Changed fields: removed

### Evidence Needed

- [ ] Screenshot or export of the relevant setting
- [ ] Date/time of validation
- [ ] System or asset name
- [ ] Reviewer notes

### Acceptance Criteria

- [ ] Change reviewed by suggested owner
- [ ] Ticket priority matches STIGPilot impact category
- [ ] Evidence request updated or confirmed unchanged
- [ ] Notes added for any downstream checklist or backlog impact

### Notes

- 

## Issue 3

Title: `severity_increased: V-CHROME-001 - Chrome Safe Browsing enhanced protection must be enabled`

Labels: `stigpilot`, `high_priority_review`, `IAM`, `Endpoint Security`, `GPO`, `Registry`, `Browser Security`

### Context

- Severity: high
- Suggested owner: Endpoint/Windows Admin
- Impact: High-priority review (`high_priority_review`)
- Reason: The severity increased to high and the remediation text changed, so old tickets or implementation notes should not be reused without review.
- Changed fields: title, severity, check_text, fix_text, references

### Evidence Needed

- [ ] Screenshot or export of the relevant setting
- [ ] GPO, registry, or Local Security Policy export
- [ ] Date/time of validation
- [ ] System or asset name
- [ ] Reviewer notes

### Acceptance Criteria

- [ ] Change reviewed by suggested owner
- [ ] Ticket priority matches STIGPilot impact category
- [ ] Evidence request updated or confirmed unchanged
- [ ] Notes added for any downstream checklist or backlog impact

### Notes

- 

## Issue 4

Title: `check_changed: V-CHROME-002 - Chrome password manager must be disabled`

Labels: `stigpilot`, `evidence_update_likely`, `Password Policy`, `GPO`, `Registry`, `Browser Security`

### Context

- Severity: medium
- Suggested owner: Endpoint/Windows Admin
- Impact: Evidence update likely (`evidence_update_likely`)
- Reason: The check procedure changed enough that evidence requests or validation steps may need to be refreshed.
- Changed fields: check_text

### Evidence Needed

- [ ] Screenshot or export of the relevant setting
- [ ] GPO, registry, or Local Security Policy export
- [ ] Date/time of validation
- [ ] System or asset name
- [ ] Reviewer notes

### Acceptance Criteria

- [ ] Change reviewed by suggested owner
- [ ] Ticket priority matches STIGPilot impact category
- [ ] Evidence request updated or confirmed unchanged
- [ ] Notes added for any downstream checklist or backlog impact

### Notes

- 
