...
> **Risk level: HIGH**: 2 control(s) require immediate review before reusing existing tickets or evidence.
...
Old source: `synthetic.xml`
New source: `synthetic.xml`
...
### 1. High-priority review V-100001: Windows audit policy must be configured and reviewed

**Impact:** High-priority review | **Owner:** Endpoint/Windows Admin

The severity increased to high and the remediation text changed, so old tickets or implementation notes should not be reused without review.

### 2. High-priority review V-100004: Firewall management access must be restricted

**Impact:** High-priority review | **Owner:** Network/Security Engineering

A new high-severity control was added, so it should be triaged before lower-risk backlog work.

### 3. Evidence update likely V-100002: Linux SSH banner must be configured

**Impact:** Evidence update likely | **Owner:** Linux Admin

The check procedure changed enough that evidence requests or validation steps may need to be refreshed.

### 4. Review recommended V-100003: Removed database audit control

**Impact:** Review recommended | **Owner:** Database Admin

The control was removed, so downstream tickets, evidence requests, or mappings may need cleanup.
