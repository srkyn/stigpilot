...
[CmdletBinding()]
param(
    [ValidateSet("help", "doctor", "parse", "diff", "evidence", "packet", "inspect", "archive")]
    [string]$Command = "help",
...
if ($PSBoundParameters.Count -eq 0) {
    ...
    Write-Host "  .\STIGPilot-Gov.ps1 -Command archive -OutDir output\packet -Zip output\packet.zip"
    Write-Host ""
...
function Write-GovHelp {
    ...
    Write-Host "  archive   Inspect a generated packet, then create a ZIP archive for handoff"
    Write-Host ""
    Write-Host "Examples:"
    ...
    Write-Host "  .\tools\STIGPilot-Gov.ps1 -Command archive -OutDir output\gov -Zip output\gov.zip"
...
function Write-PacketStartHere {
    ...
    $lines += ""
    $lines += "## Next 15 Minutes"
    $lines += ""
    $lines += "| Role | Do this first |"
    $lines += "| --- | --- |"
    $lines += "| Security analyst | Read ``change-brief.md``, then mark the high-priority and implementation-likely rows in ``remediation-backlog.csv``. |"
    $lines += "| Sysadmin or engineer | Filter ``remediation-backlog.csv`` by your owner group, then review changed fix text before reusing old implementation notes. |"
    $lines += "| GRC or evidence owner | Open ``evidence-checklist.md`` and refresh requests for controls marked evidence update likely. |"
    $lines += "| Manager or lead | Use the summary counts and top actions to decide which team needs the first review block. |"
    $lines += ""
    $lines += "## File Map"
    $lines += ""
...
function Write-PacketInspection {
    param($Inspection)
    ...
    Write-GovRule
    foreach ($row in $Inspection.Rows) {
        $color = if ($row.Status -eq "PASS") { "Green" } else { "Red" }
        Write-Host ("{0,-28} {1,-6} {2}" -f $row.File, $row.Status, $row.Details) -ForegroundColor $color
    }
    Write-GovRule

    if ($Inspection.ChangeSummary) {
        Write-Host ("Change summary: {0} total, {1} high-priority, {2} implementation-likely, {3} evidence-update-likely." -f `
            $Inspection.ChangeSummary.total, `
            $Inspection.ChangeSummary.high_priority_review, `
            $Inspection.ChangeSummary.implementation_change_likely, `
            $Inspection.ChangeSummary.evidence_update_likely)
    }
}

function Invoke-InspectOutput {
    $inspection = Get-PacketInspection
    Write-PacketInspection $inspection

    if ($inspection.Ok) {
        Write-Host "Packet is handoff-ready: $OutDir" -ForegroundColor Green
        Write-Host "Start here: $($inspection.StartHerePath)" -ForegroundColor Cyan
    }
    else {
...
function Invoke-ArchiveOutput {
    $inspection = Get-PacketInspection
    Write-PacketInspection $inspection

    if (-not $inspection.Ok) {
        Write-Host "Archive skipped because packet is incomplete: $OutDir" -ForegroundColor Red
        exit 1
    }

    $zipPath = if ($Zip) { $Zip } else { "$OutDir.zip" }
    if ((Test-Path -LiteralPath $zipPath) -and (-not $Force)) {
        throw "Archive already exists: $zipPath. Use -Force to overwrite it."
    }

    $zipParent = Split-Path -Parent $zipPath
    if ($zipParent) {
        Ensure-Directory $zipParent
    }
    ...
    Write-Host "Wrote packet archive: $zipPath" -ForegroundColor Green
    Write-Host "Archived folder: $OutDir" -ForegroundColor Cyan
}
